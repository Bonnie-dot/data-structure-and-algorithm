...
export class GoodSuffixShift {
    ...
    generateGS() {
        for (let i = 0; i < this.patternStringLength - 1; i++) {
            ...
            while (
                j >= 0 &&
                this.patterString[j] ===
                    this.patterString[this.patternStringLength - 1 - k]
            ) {
                j--;
                k++;
                this.suffix[k] = j + 1;
    ...
    matchWithBM(mainString: string) {
        ...
        while (
            matchMainStringWithPatternString <=
            mainString.length - this.patternStringLength
        ) {
            let j = this.patternStringLength - 1;
            for (; j >= 0; j--) {
                if (
                    mainString[matchMainStringWithPatternString + j] !==
                    this.patterString[j]
                ) {
                    break;
                }
            ...
            const badCharacterNeedMoveNumber =
                j -
                hashTable[
                    mainString[matchMainStringWithPatternString + j].charCodeAt(
                        0,
                    )
                ];
            let goodSuffixShiftNeedMoveNumber = 0;
            if (j < this.patternStringLength - 1) {
            ...
            matchMainStringWithPatternString =
                matchMainStringWithPatternString +
                Math.max(
                    badCharacterNeedMoveNumber,
                    goodSuffixShiftNeedMoveNumber,
                );
        }
        return -1;
    }
    ...
    moveByGoodSuffix(badCharacterIndex: number): number {
        ...
        if (this.suffix[matchLength] !== -1) {
            ...
            for (
                let i = badCharacterIndex + 2;
                i < this.patternStringLength;
                i++
            ) {
