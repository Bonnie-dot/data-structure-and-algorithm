...
export class GraphAdjacencyList {
    ...
    findShortestPath () {
      ...
      for (let i = 1; i < this.vertexCount; i++) {
        ...
        // 更新最短路径集合
        for (let j = 0; j < this.vertexCount; j++) {
          if (!markedFoundShortestPath[j] && this.adjacencyMatrix[currentMinPathIndex][j] < recordPaths[j]) {
            recordPaths[j] = min + this.adjacencyMatrix[currentMinPathIndex][j];
            shortestPathsVertexs[j] = currentMinPathIndex;
          }
        }
      }
      return { shortestPathsFromV0ToVn, shortestPathsVertexs: this.concatShortestPath(shortestPathsVertexs) };
    }
    ...
    concatShortestPath (shortestPathsVertexs:number[]) {
      ...
      shortestPathsVertexs.forEach((item, index) => {
        if (index >= 1) {
          ...
          const historyVertex = this.findFinalHasHistoryVertex(previousVertexs, currentPreviousIndex, shortestPathsVertexs, shortestPaths);
          const noRecordHistoryVertexs = historyVertex.noRecordHistoryVertexs;
          previousVertexs = historyVertex.previousVertexs;

          if (noRecordHistoryVertexs.length > 0) {
            shortestPaths.push(`${previousVertexs}-${noRecordHistoryVertexs.join('-')}-${index}`);
          } else {
    ...
    private findFinalHasHistoryVertex (previousVertexs, currentPreviousIndex: number, shortestPathsVertexs: number[], shortestPaths: any[]) {
