...
export class TopologicalWithWeight {
  ...
  initAdList(){
    this.adList.push(new VertexNode(0,"v0",new EdgeNode("v2",new EdgeNode("v1",null,3),4)));
    this.adList.push(new VertexNode(1,"v1",new EdgeNode("v4",new EdgeNode("v3",null,5),6)));
    this.adList.push(new VertexNode(1,"v2",new EdgeNode("v5",new EdgeNode("v3",null,8),7)));
    this.adList.push(new VertexNode(2,"v3",new EdgeNode("v4",null,3)));
    this.adList.push(new VertexNode(2,"v4",new EdgeNode("v7",new EdgeNode("v6",null,9),4)));
  ...
  getEarliestTimeOfVertex(){
    ...
    while (this.recordZeroInCountStack.length!==0){
      ...
      while (edge){// iterate all related edge's inCount --
        const edgeIndex = this.adList.findIndex(item=>item.name === edge.name);
        if(edgeIndex&& (--this.adList[edgeIndex].inCount)===0){
          this.recordZeroInCountStack.push(edgeIndex);
        }
        if (this.earliestTimeOfVertex[vertexIndex]+edge.weight>this.earliestTimeOfVertex[edgeIndex]){
          this.earliestTimeOfVertex[edgeIndex] = this.earliestTimeOfVertex[vertexIndex]+edge.weight;
        }
        edge = edge.next;
      }
    }
  ...
  getLatestTimeOfVertex(){
    this.latestTimeOfVertex.fill(this.earliestTimeOfVertex[this.adListLength-1]);
    let vertexIndex = this.recordSequencePopStack.pop();
    while (vertexIndex){
      let edge = this.adList[vertexIndex].firstEdge;
      while (edge){
        const edgeIndex = this.adList.findIndex(item=>item.name === edge.name);
        if (this.latestTimeOfVertex[edgeIndex]-edge.weight<this.latestTimeOfVertex[vertexIndex]) {
          this.latestTimeOfVertex[vertexIndex] = this.latestTimeOfVertex[edgeIndex]-edge.weight;
        }
        edge = edge.next;
      }
  ...
  findCriticalPath(){
    ...
    this.adList.forEach((item,index)=>{
      ...
      while (edge){
        const edgeIndex = this.adList.findIndex(item=>item.name == edge.name);
        latestTimeOfEdge = this.latestTimeOfVertex[edgeIndex]-edge.weight;
